"""Pydantic models mirroring the tau2-bench airline data model."""
...
SeatPrices = Annotated[
    Dict[CabinClass, int],
    Field(description="Prices for different cabin classes"),
]
...
class FlightDataStatusOnTime(BaseModel):
    ...
    estimated_departure_time_est: str = Field(
        description=(
            "Estimated departure time in EST in the format YYYY-MM-DDTHH:MM:SS, "
            "e.g 2024-05-15T06:04:00"
        )
    )
    estimated_arrival_time_est: str = Field(
        description=(
            "Estimated arrival time in EST in the format YYYY-MM-DDTHH:MM:SS, "
...
class FlightDataStatusFlying(BaseModel):
    ...
    actual_departure_time_est: str = Field(
        description=(
            "Actual departure time in EST in the format YYYY-MM-DDTHH:MM:SS, "
            "e.g 2024-05-15T06:04:00"
        )
    )
    estimated_arrival_time_est: str = Field(
...
class FlightDateStatusLanded(BaseModel):
    ...
    actual_arrival_time_est: str = Field(
        description=(
            "Actual arrival time in EST in the format YYYY-MM-DDTHH:MM:SS, "
            "e.g 2024-05-15T07:30:00"
        )
    )
...
class DirectFlight(FlightBase):
    ...
    date: Optional[str] = Field(
        default=None, description="Flight date in YYYY-MM-DD format"
    )
    available_seats: AvailableSeats = Field(description="Available seats by class")
    prices: SeatPrices = Field(description="Current prices by class")
...
class Reservation(BaseModel):
    ...
    status: Optional[Literal["cancelled"]] = Field(
        default=None, description="Status of the reservation"
    )


class FlightDB(BaseModel):
    """Subset of the tau2 FlightDB for schema validation."""
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """Return simple counts matching the tau2 helper."""

        num_flights = len(self.flights)
        num_flight_instances = sum(len(flight.dates) for flight in self.flights.values())
